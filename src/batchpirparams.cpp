...
BatchPirParams::BatchPirParams(int batch_size, bool parallel, BatchPirType type, uint64_t pirana_k)
    ...
    if (type_ == UIUC) {
        ...
        response_size = {size_t(NumHashFunctions),  num_servers * num_chunk_ctx};
    } else {
        ...
        response_size = {size_t(NumHashFunctions),  get_num_slots_per_entry()};
    }
...
void BatchPirParams::set_first_dimension_size(){
    ...
    auto batch_size = ceil((batch_size_*CuckooFactor)*1.0/2);
